...
def parse_redcap_event(event_name: str) -> Tuple[int, int]:
    ...
    if matches is None:
        raise Exception(
            f"Unable to parse the string: '{event_name}' as a redcap event, please check the code / clean the data"
        )
...
def merge_rows(selected_rows):
    ...
    for column_name in list(selected_rows.columns)[2:]:
        ...
        if not_null_count > 1:
            ...
            logging.info(
                f"Moved data for column name: {column_name} for hhid: {new_row['hhid']}"
            )
...
def extend_columns(data_frame):
    ...
    for redcap_event in unique_redcap_events:
        ...
        for old_column_name in list(COLUMNS_TO_DELETE):
            # Make the new name
            new_columns.append(
                generate_column_name(visit_number, arm_number, old_column_name)
            )
...
def move_data(current_hhid, selected_rows, data_frame):
    ...
    for column_name in list(COLUMNS_TO_DELETE):
        ...
        for row_index, value in enumerate(column_data):
            ...
            new_column_name = generate_column_name(
                visit_number, arm_number, column_name
            )
            ...
            if new_column_name in data_frame.columns:
                ...
                logging.info(
                    f"Moved data to a new column: {new_column_name} for hhid: {current_row['hhid']} arm number: {arm_number} and value: {value}"
                )
            else:
...
def merge_redcap_event_rows(filename: str):
    ...
    for i, current_hhid in enumerate(unique_hhids):
        ...
        main_logger.info(f"Appending the row for HHID: {new_row['hhid']}")
    ...
    trimmed_df.to_csv(
        f"/root/sandbox/disa/data_merged_{str_date_time}.tsv", sep="\t", index=False
    )

    main_logger.info("Deleted Columms:")
    main_logger.info(COLUMNS_TO_DELETE)
